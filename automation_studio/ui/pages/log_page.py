...
class LogPage(QtWidgets.QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        filters.addWidget(self._labeled_field("Watcher", self.watcher_filter))
        ...
        self.table.setHorizontalHeaderLabels(
            ["Time", "Level", "Workflow", "Device", "Watcher", "Status", "Message", "Metadata"]
        )
    ...
    def refresh_filters(self) -> None:
        ...
        watcher_index = self.watcher_filter.findData(current_watcher)
        if workflow_index >= 0:
            self.workflow_filter.setCurrentIndex(workflow_index)
        if device_index >= 0:
            self.device_filter.setCurrentIndex(device_index)
        if watcher_index >= 0:
            self.watcher_filter.setCurrentIndex(watcher_index)
